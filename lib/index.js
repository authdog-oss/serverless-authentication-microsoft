...
var MicrosoftProvider = function (_Provider) {
  _inherits(MicrosoftProvider, _Provider);

  function MicrosoftProvider() {
    _classCallCheck(this, MicrosoftProvider);

    return _possibleConstructorReturn(this, Object.getPrototypeOf(MicrosoftProvider).apply(this, arguments));
  }

  _createClass(MicrosoftProvider, [{
    key: 'signinHandler',
    value: function signinHandler(_ref, callback) {
      var _ref$scope = _ref.scope;
      var scope = _ref$scope === undefined ? 'wl.basic' : _ref$scope;
      var state = _ref.state;

      var options = Object.assign({ scope: scope, state: state }, { signin_uri: 'https://login.live.com/oauth20_authorize.srf', response_type: 'code' });

      _get(Object.getPrototypeOf(MicrosoftProvider.prototype), 'signin', this).call(this, options, callback);
    }
  }, {
    key: 'callbackHandler',
    value: function callbackHandler(event, callback) {
      ...
      _get(Object.getPrototypeOf(MicrosoftProvider.prototype), 'callback', this).call(this, event, options, { authorization: { grant_type: 'authorization_code' } }, callback);
    }
  }]);
...
var signinHandler = function signinHandler(config, options, callback) {
  return new MicrosoftProvider(config).signinHandler(options, callback);
};

var callbackHandler = function callbackHandler(event, config, callback) {
  return new MicrosoftProvider(config).callbackHandler(event, callback);
};
